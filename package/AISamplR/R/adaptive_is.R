...
#'
#' \code{lais}, \code{apis} and \code{pmc} respectively return samples and the associated weights
...
#' Adaptive Population Importance Sampling (APIS) and Population Monte Carlo (PMC). Theses 3 implementations follows the same steps:
#' \enumerate{
#'   \item the adaptation step where a set of N proposal distributions are update at each iteration t for t = 1,..., T.
...
#' The covariance matrix of theses proposal distribution are equal and provided by the user (argument \code{sig2_samp}). 
...
#' the location parameters mu_\{t, n\} the proposal are updated.
#' \itemize{
#'   \item \code{lais} uses a Markov chain Monte Carlo with the Metropolis-Hasting algorithm. 
#'   \item \code{apis} takes, as a new location parameter mu_\{t,n\},
...
#'   \item \code{apis} takes, as location parameter mu_\{t,n\},
#'   the weighted mean of M samples drawn from a the  proposal distribution with location parameter mu_\{t-1,n\}.
#'   Each sample has a different probability of being chosen.
...
##' }
#' 
#' @param logposterior is either of function of a pointer to a C++ function.
#' Those functions have to take as argument an numercial vector of a given length D.
#' This argument corresponds to the log-likelihood of the posterior distribution, we want to draw samples from.
#' The algorithm used in the adapation step are iterative. 
#' Therefore, this step are quite slow in R. 
...
#' @param compute_logdenom A function that indicates how to compute denominators for the weights. 
#' @param N An integer indicating the number of chains of proposal distributions that we want to use.
#' @param T An integer indicating the number of iterations used for the adaptation of the N chains of proposal distributions. 
#' It corresponds to the length of the chain of proposal distributions.
#' @param M An integer indicating the numbers of samples to be drawn for each proposal distribution.
...
#'   drawn from the proposal distribution in the sampling step.}
#'   \item{loglik}{an array of dimension T x M x N with
#'   the loglikelihood of the posterior distribution 
#'   evaluated for each sample drawn im \code{x}.} 
